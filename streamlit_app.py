...
def fetch_cyclon_logo():
    """Find and download the current CYCLON logo from the official CYCLON site."""
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        r = requests.get("https://www.cyclon-lpc.com/", headers=headers, timeout=15)
        r.raise_for_status()
        ...
        for img in soup.find_all("img"):
            ...
            low = src.lower()
            if src and ("logo" in low or "cyclon" in alt):
                if src.startswith("//"):
                    src = "https:" + src
                elif src.startswith("/"):
                    src = "https://www.cyclon-lpc.com" + src
                candidates.append(src)
        # Prefer a file whose URL explicitly looks like a logo.
        candidates.sort(key=lambda u: ("logo" not in u.lower(), len(u)))
        for url in candidates:
            try:
                rr = requests.get(url, headers=headers, timeout=15)
                ctype = rr.headers.get("content-type", "")
                if rr.ok and rr.content and "image" in ctype:
                    return rr.content
            except Exception:
                pass
    except Exception:
...
logo_bytes = fetch_cyclon_logo()
if logo_bytes:
    st.image(logo_bytes, width=220)
else:
    # Visible branded fallback if the official site is temporarily unavailable.
    st.markdown(
        '<div style="display:inline-block;background:#122a54;color:#ffd400;'
        'font-size:34px;font-weight:900;padding:10px 20px;border-radius:3px;'
        'margin-bottom:10px;letter-spacing:1px;">CYCLON</div>',
        unsafe_allow_html=True,
    )
...
@st.cache_data(ttl=86400, show_spinner=False)
def image_as_data_uri(url):
    """
    Download official CYCLON product images on the Streamlit server and embed
    them in the result table. This avoids browser hot-link blocking.
    """
    if not url:
        return ""
    try:
...
def render_table(items, lookup):
    parts = ['<div class="oil-table-wrap"><table class="oil-table"><thead><tr>',
             ...
             '</tr></thead><tbody>']
    for item in items:
        img = image_as_data_uri(image_url(item))
